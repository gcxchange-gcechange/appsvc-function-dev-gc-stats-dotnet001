...
public class ProcessTotalFiles
{
    private readonly ILogger<ProcessTotalFiles> _logger;
    private readonly IConfiguration _config;

    public ProcessTotalFiles(ILogger<ProcessTotalFiles> logger, IConfiguration config)
    {
        _logger = logger;
        _config = config;
    }

    [Function("ProcessTotalFiles")]
    public async Task Run([QueueTrigger("process-total-files", Connection = "")] string blobName)
    {
        ...
        try
        {
            ...
            var containerClient = blobServiceClient.GetBlobContainerClient(TotalFiles.FilesContainerName);
            var blobClient = containerClient.GetBlobClient(blobName);

            if (!await blobClient.ExistsAsync())
                throw new FileNotFoundException($"Blob {blobName} not found in container {TotalFiles.FilesContainerName}");
            ...
            _logger.LogInformation("Successfully uploaded items from {blobName} to dbo.Files", blobName);
        }
        catch (Exception ex)
        {
